...
// Rotas de API compartilhadas entre o servidor tradicional (server.ts, usado no
// Google Cloud Run / dev local) e a função serverless da Vercel (api/index.ts).
// Manter a lógica num único lugar evita que uma correção seja aplicada em um
// ambiente e esquecida no outro.
    ...
    try {
      ...
      if (contentType.includes("text/html") && fileId) {
        ...
        if (confirmMatch) {
          ...
          // If response was HTML but had no confirm token, we cannot recover: this usually
          // means Google's per-file abuse quota kicked in ("too many downloads"). Surface a
          // clear error instead of silently sending broken HTML as if it were the ROM.
          console.error("[ROM PROXY] Google Drive returned an HTML page without a confirm token (likely per-file quota/abuse block).");
          return res.status(429).send("API Error: A fonte deste arquivo (Google Drive) recusou o download no momento (provavelmente excesso de acessos recentes a este arquivo específico). Tente novamente mais tarde.");
        }
      }
